// Package "issue280" provides primitives to interact with the AsyncAPI specification.
//
...
func brokerMessageToTestMessageMessageFromTestingChannel(bMsg extensions.BrokerMessage) (TestMessageMessageFromTestingChannel, error) {
	...
	for k, v := range bMsg.Headers {
		switch {
		case k == "kafka_correlationId": // Retrieving KafkaCorrelationId header
			h := string(v)
			msg.Headers.KafkaCorrelationId = &h
		case k == "kafka_key": // Retrieving KafkaKey header
			h := string(v)
			msg.Headers.KafkaKey = &h
		case k == "kafka_timestamp": // Retrieving KafkaTimestamp header
			t, err := time.Parse(time.RFC3339, string(v))
			if err != nil {
				return msg, err
			}
			msg.Headers.KafkaTimestamp = &t
		default:
...
func (msg TestMessageMessageFromTestingChannel) toBrokerMessage() (extensions.BrokerMessage, error) {
	...
	// Adding KafkaCorrelationId header
	if msg.Headers.KafkaCorrelationId != nil {
		headers["kafka_correlationId"] = []byte(*msg.Headers.KafkaCorrelationId)
	}

	// Adding KafkaKey header
	if msg.Headers.KafkaKey != nil {
		headers["kafka_key"] = []byte(*msg.Headers.KafkaKey)
	}

	// Adding KafkaTimestamp header
	if msg.Headers.KafkaTimestamp != nil {
		headers["kafka_timestamp"] = []byte(msg.Headers.KafkaTimestamp.Format(time.RFC3339))
	}
...
type KafkaHeadersSchema struct {
	KafkaCorrelationId *string    `json:"kafka_correlationId,omitempty"`
	KafkaKey           *string    `json:"kafka_key,omitempty"`
	KafkaTimestamp     *time.Time `json:"kafka_timestamp,omitempty"`
}
...
const (
	...
	TestingChannelPath = "v3.issue280.test"
)
